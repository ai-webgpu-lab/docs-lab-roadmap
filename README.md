...
- `.github/workflows/operations-check.yml` — Pages/README/Workflow/Project 상태를 한 번에 검증하는 수동 운영 점검 workflow
...
운영 점검 workflow는 org 전체 저장소와 Projects v2를 읽어야 하므로 `AI_WEBGPU_LAB_ADMIN_TOKEN` repository secret에 `repo`, `read:org`, Projects 접근 권한이 있는 토큰을 설정한 뒤 실행합니다. 기본값은 Project field dry-run이며, drift를 실제로 복구할 때만 `apply_project_fields=true`로 실행합니다.
