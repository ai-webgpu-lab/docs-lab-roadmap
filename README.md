...
- `scripts/render-projects-config.mjs` — Projects v2 board + 시드 issue 30개 dry-render (`--apply <script.sh>`로 gh CLI 스크립트 emit)
...
- `tests/run-all.sh` — `tests/test-*.sh` 전수 실행 wrapper (`--filter <pattern>`, `--bail`, `--quiet` 지원, pass/fail 요약과 elapsed 출력)
