...
function buildResult() {
  ...
  return {
    meta: {
      ...
      owner: "ai-webgpu-lab",
      track: "benchmark",
      scenario: run ? `embeddings-latency-quality-${run.executionMode}` : "embeddings-latency-quality-pending",
      notes: winner
        ? `winner=${winner.profile.id}; docs=${DOCUMENTS.length}; queries=${QUERIES.length}; mrr=${round(winner.mrr, 4)}; executionMode=${run.executionMode}; backend=${state.environment.backend}`
