...
function buildResult() {
  ...
  return {
    meta: {
      ...
      owner: "ai-webgpu-lab",
      track: "llm",
      scenario: run ? `webllm-browser-chat-readiness-${executionMode.id}` : "webllm-browser-chat-pending",
      notes: run
        ? `promptTokens=${run.promptTokens}; outputTokens=${run.outputTokens}; executionMode=${executionMode.id}; backend=${executionMode.backend}`
