...
function buildResult() {
  ...
  return {
    meta: {
      ...
      owner: "ai-webgpu-lab",
      track: "benchmark",
      scenario: run ? `voice-roundtrip-${run.executionMode}` : "voice-roundtrip-pending",
      notes: winner
        ? `winner=${winner.profile.id}; wakeWord=${winner.wakeWord}; firstAudioMs=${round(winner.firstAudioMs, 2)}; voice=${state.fixture.ttsVoice}; executionMode=${run.executionMode}; backend=${state.environment.backend}`
