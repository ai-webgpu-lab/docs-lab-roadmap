...
async function awaitRealBenchmark(timeoutMs = REAL_ADAPTER_WAIT_MS) {
  ...
  while (performance.now() - startedAt < timeoutMs) {
    ...
    if (typeof window !== "undefined" && window.__aiWebGpuLabRealAtomicsBenchBootstrapError) {
      return null;
    }
...
async function runRealBenchmarkAtomics(adapter) {
  log(`Connecting real benchmark adapter '${adapter.id}'.`);
  await withTimeout(
    Promise.resolve(adapter.createBenchmark({ name: "atomics-memory" })),
    REAL_ADAPTER_LOAD_MS,
    `createBenchmark(${adapter.id})`
  );
  await withTimeout(
    Promise.resolve(adapter.runProfile({
      profileId: "atomics-memory-default",
      fn: () => null,
      options: {}
    })),
    REAL_ADAPTER_LOAD_MS,
...
async function runBenchmark() {
  ...
  if (isRealBenchmarkMode) {
    ...
    if (adapter) {
      try {
        const { aggregate } = await runRealBenchmarkAtomics(adapter);
      ...
      const reason = (typeof window !== "undefined" && window.__aiWebGpuLabRealAtomicsBenchBootstrapError) || "timed out waiting for adapter registration";
      state.realAdapterError = reason;
      log(`No real benchmark adapter registered (${reason}); falling back to deterministic atomics benchmark.`);
    }
  }
...
function describeBenchmarkAdapter() {
  ...
  return {
    id: "deterministic-atomics-memory",
    label: "Deterministic Atomics and Memory",
...
function buildResult() {
  ...
  return {
    meta: {
      ...
      owner: "ai-webgpu-lab",
      track: "benchmark",
      scenario: (state.run && state.run.realAdapter) ? `atomics-and-memory-real-${state.run.realAdapter.id}` : (run ? "atomics-and-memory-benchmark" : "atomics-and-memory-pending"),
      notes: run
        ? `cases=${run.overall.caseCount}; winner=${run.overall.winnerId}; backend=${state.environment.backend}; peakItems=${run.overall.maxBodiesOrParticles}; maxWorkgroup=${run.overall.maxWorkgroupSize}; sharedMemory=${run.overall.maxSharedMemoryKb}; suite=${state.fixture?.id || INLINE_FIXTURE.id}${state.run && state.run.realAdapter ? `; realAdapter=${state.run.realAdapter.id}` : (isRealBenchmarkMode && state.realAdapterError ? `; realAdapter=fallback(${state.realAdapterError})` : "")}`
