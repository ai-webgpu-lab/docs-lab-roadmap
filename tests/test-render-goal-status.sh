...
INCOMPLETE_BENCHMARK_DOC="${TMP_DIR}/incomplete-benchmark.md"
...
assert_contains "${CONTENT}" "Benchmark raw fixture links | 10/10 | >=10 | ✅"
...
cat >"${INCOMPLETE_BENCHMARK_DOC}" <<'MD'
# Benchmark Summary
...
INCOMPLETE_BENCHMARK_OUTPUT="$(node "${REPO_ROOT}/scripts/render-goal-status.mjs" \
  --benchmark-summary "${INCOMPLETE_BENCHMARK_DOC}" \
  --stdout)"
assert_contains "${INCOMPLETE_BENCHMARK_OUTPUT}" "Benchmark summary v1 | ⚠ incomplete"
assert_contains "${INCOMPLETE_BENCHMARK_OUTPUT}" "3/4 decision/report artifacts complete"
assert_contains "${INCOMPLETE_BENCHMARK_OUTPUT}" "generated by scripts/render-benchmark-summary.mjs"
assert_contains "${INCOMPLETE_BENCHMARK_OUTPUT}" "at least 10 raw result fixtures"
