...
SKETCH_PATH="${REPO_ROOT}/repo-scaffolds/repos/exp-fluid-webgpu-core/public/real-renderer-sketch.js"
...
for (const exportName of ["connectRealFluid", "buildRealFluidAdapter", "loadWebGpuFromBrowser"]) {
...
const adapter = sketch.buildRealFluidAdapter({ device: stubDevice, version: "raw-webgpu-1" });
if (!adapter || adapter.id !== "fluid-rawgpu-1") {
...
const scene = await adapter.loadScene({ count: 512 });
if (scene.count !== 512) {
  console.error("FAIL: loadScene returned wrong count", scene);
  process.exit(1);
}

const result = await adapter.renderFrame({ frameIndex: 3 });
if (result.workgroups !== Math.ceil(512 / 64) || result.particleCount !== 512) {
...
const connect = await sketch.connectRealFluid({ registry, loader: stubLoader, version: "raw-webgpu-1" });
if (connect.adapter.id !== "fluid-rawgpu-1") {
  console.error("FAIL: connectRealFluid returned wrong adapter");
  process.exit(1);
}
const stub = registry.describe("adapter-stub");
if (stub.status !== "connected" || stub.id !== "fluid-rawgpu-1") {
  console.error("FAIL: stub describe should reflect connected fluid", stub);
  process.exit(1);
}

let threwOnMissingRegistry = false;
try { await sketch.connectRealFluid({ registry: null, loader: stubLoader }); } catch (error) { threwOnMissingRegistry = true; }
if (!threwOnMissingRegistry) { console.error("FAIL: connectRealFluid should throw without registry"); process.exit(1); }

let threwOnBadDevice = false;
try { sketch.buildRealFluidAdapter({ device: null }); } catch (error) { threwOnBadDevice = true; }
if (!threwOnBadDevice) { console.error("FAIL: buildRealFluidAdapter should throw without device"); process.exit(1); }

console.log("OK real fluid sketch");
EOF

OUTPUT="$(node "${DRIVER}" "${ADAPTER_PATH}" "${SKETCH_PATH}" 2>&1)"
echo "${OUTPUT}"
if [[ "${OUTPUT}" != *"OK real fluid sketch"* ]]; then
  echo "test failed: missing success line" >&2
  exit 1
fi

echo "real-fluid-sketch test passed"
